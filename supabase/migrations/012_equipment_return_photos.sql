...
create or replace function public.save_equipment(p_data jsonb) returns uuid language plpgsql security definer set search_path='' as $$
 ...
 select * into manager from public.profiles where id=(p_data->>'manager_id')::uuid and role in ('admin','super_admin') and membership_status='approved' and not banned and (suspended_until is null or suspended_until<=now());
