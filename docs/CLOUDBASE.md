...
当前采用用户名密码认证，邮箱用作平台登录标识。注册时仍由超级管理员审核成员身份；找回密码时必须完成注册邮箱验证码验证。密码为 10–32 位，首位为英文字母或数字，至少包含大写字母、小写字母、数字、允许的特殊字符中的三类。密码由 CloudBase 管理，业务数据库不保存密码。
...
登录页的“忘记密码”进入 `#/reset-password`。使用 CloudBase 原生 `getVerification({ email, usage: 'recovery', target: 'USER' })` 请求验证码，`verify` 核验后取得短期凭证，再通过 `resetPassword` 设置新密码。验证码、验证凭证与新密码不写入业务数据库、URL 或浏览器持久存储；重置完成后退出当前会话，用户重新登录。角色、准入审核状态及预约记录不受影响。

2026-09-15 已开启 `email` 身份源及腾讯云默认邮件代发，保留原用户名登录。维护接口为 `ModifyProvider`（API 版本 `2018-06-08`），本次变更参数如下：

```json
{
  "EnvId": "hclab-equipments-d0ehkkk05991a35",
  "Id": "email",
  "On": "TRUE",
  "EmailConfig": { "On": "TRUE" }
}
...
`EmailConfig.On = TRUE` 表示平台默认代发，详见[腾讯云官方配置说明](https://cloud.tencent.com/document/api/876/34822#EmailProviderConfig)。以后若使用自己的发信邮箱，应在控制台配置 SMTP，授权码只能保存在云端。不要覆盖已有的自定义 SMTP 配置，也不要为了找回密码开启自动注册或自动账号合并。

页面提供 60 秒重发间隔；验证码有效期、频率限制和核验由云端决定。不存在的账号不触发注册，验证码无效或过期不会修改密码。现有原生账号邮箱与平台资料已核对一致。演示模式不模拟发信，Supabase 备选适配器尚未接入此入口。

发信接口成功只表示请求已被接受，并不证明邮件已到达收件箱。若收不到，先检查垃圾邮件及学校邮箱隔离区；仍未送达时，在腾讯云控制台排查邮件通道或联系其支持提供投递记录，不应跳过验证码直接重置密码。当前测试请求返回成功、有效期为 600 秒，收件人已确认收到验证码。真实账号的密码未在验收中修改，实际改密由账号本人操作。
